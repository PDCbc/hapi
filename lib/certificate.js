...
var async = require('async'),
    _ = require('lodash'),
    logger = require('./logger').Logger("certificate", 1);
...
function certificate(next) {
    ...
    async.auto({
        key: function (next) {
            fs.readFile('./cert/server.key', 'utf8', next);
        },
        cert: function (next) {
            fs.readFile('./cert/server.crt', 'utf8', next);
        }
    }, function (error, results) {
        // If there are no certificates, generate some.
        if (error) {
            generateCertificate(error, results, next);
        }
