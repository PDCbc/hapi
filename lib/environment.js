...
function environment(next) {
    ...
    if (!process.env.MONGO_URI) {
      ...
      logger.warn('No $MONGO_URI present. Defaulting to `mongodb://localhost:27019/query_composer_development`.');
      process.env.MONGO_URI = 'mongodb://localhost:27019/query_composer_development';
    }
    else
    {
    ...
    if (!process.env.AUTH_CONTROL) {
        logger.warn("No $AUTH_CONTROL present. Defaulting to `https://localhost:8081`");
        process.env.AUTH_CONTROL = "https://localhost:8081";
    } else {
