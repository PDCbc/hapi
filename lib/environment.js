...
function environment(next) {
    ...
    if (process.env.HAPI_GROUPS) {

        logger.success("$HAPI_GROUPS provided: " + process.env.HAPI_GROUPS);

    } else {

        logger.warn("No $HAPI_GROUPS provided, using `groups.json` as default file");
        process.env.HAPI_GROUPS = "groups.json";
