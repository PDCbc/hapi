...
if (process.env.GROUPS) {
    ...
    try {
        ...
        logger.info("Reading groups from: " + process.env.GROUPS);
        groups = JSON.parse(fs.readFileSync(process.env.GROUPS, 'utf8'));

    } catch (e) {

        logger.error("Failed to read groups from: " + process.env.GROUPS + " falling back on default test groups.");
        logger.error(e);
    ...
    logger.warn("process.env.GROUPS not set, using default groups.");
    groups = defaultGroups;

}

logger.info("Groups are: " + require('util').inspect(groups, false, null));
