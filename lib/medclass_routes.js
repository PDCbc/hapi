...
function medclass_routes(next, data) {
  ...
  router.route('/').get(
    ...
    function (req, res)
    {
      ...
      logger.warn(util.inspect(req.session.user));
      req.params.clinic = req.session.user.clinic;
      req.params.clinician = req.session.user.clinician;
      logger.success(util.inspect(req.params));


      var dates = {};
      logger.success("medclass/")
  ...
  function fetch_medclass_execution_data(req, res, data, dates, stage, callback)
  {
    data.models.query.find({title:'PDC-055'}).exec(
        function (err, queries)
        {
          if(err)
          {
            logger.error('execution data: ' + err);
          ...
          if(queries.length!=1)
          {
            ...
            logger.error('execution data: ' + errorMessage);
          ...
          if(callback)
          {
            callback(req, res, data, function (fields) {
              return fields[2]===req.params.clinician;
            }, dates, 'clinician', clinic);
          }
        });
  }
  ...
  function transformForNetwork(req, res, data, dates, result, callback)
  {
    ...
    if(clinicianCodes.length > 0)
    {

      for(var i=0; i<clinicianCodes.length; i++)
      {
        var entry={};
        ...
        networkEntry.denominator = networkCodes.reduce( reduceCodes, 0 );
        ...
        clinicEntry.denominator = clinicCodes.reduce( reduceCodes, 0 );
        ...
        clinicianEntry.denominator = clinicianCodes.reduce( reduceCodes, 0 );
    ...
    {
      res.result = {'error':'clinician has no patient with an active medication'};
    }

    function reduceCodes(previousValue, currentValue)
    {
      return previousValue + currentValue[Object.keys(currentValue)[0]];//drug name is first
    }
  ...
  function mapReduceKeysForNetwork(req, res, data, dates, callback)
  {
    data.models.query.find({title:'PDC-055'}).exec(
      function(err, queries)
      {
        ...
        if( err )
        {
          logger.error('mapReduceKeysForNetwork 1: ' + err);
          res.status(500).json({error:err});
          return;
        }
        else if(queries.length != 1)
        {
          ...
          logger.error('mapReduceKeysForNetwork 2: ' + errorMessage);
          res.status(500).json({error:errorMessage});
          return;
        }
        else
        {
          ...
          if(!query.executions)
          {
            ...
            logger.error('mapReduceKeysForNetwork 3: ' + errorMessage);
            res.status(500).json({error:errorMessage});
            return;
          }
          else if(query.executions.length < 1)
          {
            ...
            logger.error('mapReduceKeysForNetwork 4: ' + errorMessage);
            res.status(500).json({error:errorMessage});
            return;
          }
          else
  ...
  var services = {'whoatc':process.env.DCLAPI_URI + '/classbyatc/',
                  'hc-din':process.env.DCLAPI_URI + '/classbydin/',
                  'fddbs':null};
  ...
  function splitIntoCodes(element, values, filter, field)
  {
    ...
    for( var y in values )
    {
      ...
      if(filter(fields))
      {
        ...
        code.service = services[fields[1].toLowerCase()];
        codes.push(code);
      }
    }
  ...
  function mapReduceKeys(req, res, data, filter, dates, segment, callback)
  {
    data.models.result.find({execution_id: {$in : execution_ids}, endpoint_id: req.params.clinic}).exec(
      function(err, results)
      {
        ...
        if(err)
        {
          logger.error('mapReduceKeys 1: ' + err);
          res.status(500).json({error:err});
          return;
        }
        else if(!results)
  ...
  function mapReduceKeysSharedStage(res, req, data, filter, dates, segment, codes, callback)
  {
    ...
    async.map(codes,
        ...
        if(x.service)
        {
          ...
          request.get({ url: x.service + keys[0], json: true },
            function (error, res, body)
            {
              if(error)
              {
                logger.warn('res: ' + util.inspect(res, false, null));
              ...
              {
                logger.warn('status: ' + res.statusCode);
                logger.warn('href: ' + res.request.uri.href);
                logger.warn('body: ' + util.inspect(body, false, null));
                callback(null, {'UNKNOWN':1});//null code because
                return;
              }
            });
        }
      ...
      function(err, results)
      {
        if(err)
        {
          logger.error('sharedStage 2: ' + err);
          res.status(500).json({error:err});
          return;
        }
        else
