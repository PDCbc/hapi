...
function medclass_routes(next, data) {
  ...
  var services = {'whoatc':'http://dclapi:3007/classbyatc/',
                  'hc-din':'http://dclapi:3007/classbydin/',
                  'fddbs':null};
  ...
  function splitIntoCodes(element, values, filter, field)
  {
    ...
    for( var y in values )
    {
      ...
      if(filter(fields))
      {
        ...
        console.log('services field: ' + fields[1]);
  ...
  function mapReduceKeysSharedStage(res, req, data, filter, dates, segment, codes, callback)
  {
    ...
    async.map(codes,
      function (x, callback)
      {
        ...
        if(x.service)
        {
          console.log('service call: ' + x.service + keys[0]);
          ...
          request.get({ url: x.service + keys[0], json: true },
            function (error, request, body)
            {
              if(error)
              {
                logger.error('mapReduceKeysSharedStage 1: ' + error);
                callback(error, x);
                return;
              }

              logger.success('body: '+ util.inspect( body, false, null ));
