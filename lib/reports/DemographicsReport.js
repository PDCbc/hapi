...
var DemographicsReport = function (shortName, name, dependancies, proc) {
    ...
    var generateCSVReport = function (data, metadata) {
	    ...
	    logger.error('Demographics Report Error: dependencies not satisfied');
            return null;
	    ...
	    logger.error('Demographics Report Error: null data element');
            return null;
        ...
        //get the first execution that is on the designated date.
	    ...
	    logger.error('Demographics Report Error - missing start: ' +
			'[cStart: ' + cStart + ', ' + 
			'gStart: ' + gStart + ', ' +
			'nStart: ' + nStart);
            return null;
	    ...
	    logger.error('misaligned start times');
            return null;
	...
	clinician = proc.getExecutionsSeparatedByOneMonth(cStart, clinician.slice(1, clinician.length));
        group     = proc.getExecutionsSeparatedByOneMonth(gStart, group.slice(1, group.length));
        network   = proc.getExecutionsSeparatedByOneMonth(nStart, network.slice(1, network.length));
	    ...
	    logger.error('missaligned execution cardinality');
            return null

        }
	
	var dataSets = [clinician, group, network];
	
	dataSets.forEach(
		function(x)
		{
			x = x.sort(sortDecending);
		}
	);
