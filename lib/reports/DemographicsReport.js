...
var DemographicsReport = function (shortName, name, proc) {
    ...
    var generateCSVReport = function (data) {
        ...
        for (var i = 0; i < numExecutions; i++) {
            ...
            tmp = new Date(clinician[i].time * 1000);
        ...
        clinician = JSON.parse(JSON.stringify(clinician));

        try {

            ageRanges = Object.keys(clinician[0][Object.keys(clinician[0])[0]]);
            genders   = Object.keys(clinician[0]);
        ...
        for (var r = 0; r < ageRanges.length; r++) {
            ...
            for (var g = 0; g < genders.length; g++) {
                ...
                for (var e = 0; e < clinician.length; e++) {

                    s += clinician[e][genders[g]][ageRanges[r]] + ",";

                }

                s += "\n";

                if (g !== genders.length - 1) {
        ...
        logger.success(util.inspect(clinician, false, null));
