...
var PolypharmacyReport = function(shortName, name, proc){
    ...
    */
    ...
    var generateCSVReport = function(data){
        ...
        logger.success(util.inspect(data, false, null));
        ...
        for( var i = 0; i < fields.length; i++ ){
            ...
            num = data['PDC-1178'][f][0]["aggregate_result"]["numerator"];
            den = data['PDC-1178'][f][0]["aggregate_result"]["denominator"];

            s += num+",";
            s += ((num/den)*100)+",";

            num = data['PDC-053'][f][0]["aggregate_result"]["numerator"];
            den = data['PDC-053'][f][0]["aggregate_result"]["denominator"];

            s += num+",";
            s += ((num/den)*100)+",";

            num = data['PDC-054'][f][0]["aggregate_result"]["numerator"];
            den = data['PDC-054'][f][0]["aggregate_result"]["denominator"];
