    ...
    var generateCSVReport = function (data, metadata) {
        ...
        for (var k in data['PDC-1178'].anonymous) {
            ...
            try {
                ...
                num = data['PDC-1178'].anonymous[k][data['PDC-1178'].anonymous[k].length - 1]["aggregate_result"]["numerator"];
                den = data['PDC-1178'].anonymous[k][data['PDC-1178'].anonymous[k].length - 1]["aggregate_result"]["denominator"];

                s += num + ",";
                s += ((num / den) * 100) + ",";

                num = data['PDC-053'].anonymous[k][data['PDC-053'].anonymous[k].length - 1]["aggregate_result"]["numerator"];
                den = data['PDC-053'].anonymous[k][data['PDC-053'].anonymous[k].length - 1]["aggregate_result"]["denominator"];

                s += num + ",";
                s += ((num / den) * 100) + ",";

                num = data['PDC-054'].anonymous[k][data['PDC-054'].anonymous[k].length - 1]["aggregate_result"]["numerator"];
                den = data['PDC-054'].anonymous[k][data['PDC-054'].anonymous[k].length - 1]["aggregate_result"]["denominator"];
                ...
                logger.warn("Caught an error trying to put data for anonymous provider in to report, skipping this provider.");
            }
