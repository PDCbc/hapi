...
var PopulationHealthReport = function (shortName, name, proc) {
    ...
    var generateCSVReport = function (data, metadata) {
        ...
        var s = "Query Identifier,Metric Title,Description,Query Executed,% Patients,Numerator,Denominator,Target,Reference\n";
        ...
        for (var q = 0; q < proc.depends.length; q++) {
            ...
            if (data[k] && data[k]['clinician']) {
                ...
                if (
                    num === null || //num is allowed to 0, can't just do !num
                    num === undefined || !den
                ) {
