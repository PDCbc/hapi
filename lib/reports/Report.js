        ...
        request.get(
            ...
            function( err, response, body ){
                ...
                try{
                        ...
                        switch ( response.statusCode ){
                            ...
                            case 200:
                                ...
                                if ( body && body.processed_result ){
                                    ...
                                    logger.warn("Report.make_request() to /api/processed_result/"+title+ " returned status 200, but had invalid body.");
                            ...
                            case 204: 
                                ...
                                logger.warn("Report.make_request() to  /api/processed_result/"+title+" returned 204 no content available.");
                                return next(204, null);
                                break; 

                            case 400:

                                logger.warn("Report.make_request() to  /api/processed_result/"+title+" returned 400 malformed request.");
                                return next(400, null);
                                break;

                            case 404:

                                logger.warn("Report.make_request() to  /api/processed_result/"+title+" returned 404 the requested query does not exist.");
                                return next(404, null);
                                break;

                            case 401:

                                logger.warn("Report.make_request() to  /api/processed_result/"+title+" returned 401 user's credentials expired.");
                                return next(401, null);
                                break; 

                            case 500:

                                logger.warn("Report.make_request() to  /api/processed_result/"+title+" returned 500 some error occurred.");
                                return next(500, null);
                                break;

                            default:

                                logger.warn("Report.make_request() to  /api/processed_result/"+title+" returned status: "+response.statusCode+" unable to handle this case.");
                                return next(500, null);
                                break;
                    ...
                    logger.error("Report.make_request() to /api/processed_result/"+title+ " failed due to an exception: "+ util.inspect(e, false, null));
                    return next(500, null); 
