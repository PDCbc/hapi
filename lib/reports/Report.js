...
var Report = function (shortName, name, dependancies, proc) {
    ...
    *           }
    ...
    var checkTemporalSeperationOfResults = function (data) {
        ...
        return Math.abs(max_date - min_date) > 604800 ? false : true;
    ...
    var checkResult = function (result) {
        ...
        if (
            ...
            result.clinician.length <= 0 || !result.group ||
            result.group.length <= 0 || !result.network ||
    ...
    var validReport = function (req, next) {
        ...
        // 
        if (Math.abs((new Date()).getTime() - last_cached_date) < 60000) {
