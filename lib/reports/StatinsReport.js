    ...
    var generateCSVReport = function (data, metadata) {
        ...
        for (var k in data['PDC-1178'].anonymous) {
            ...
            try {
                ...
                num = data['PDC-057'].anonymous[k]["aggregate_result"]["numerator"];
                den = data['PDC-057'].anonymous[k]["aggregate_result"]["denominator"];

                s += num + ",";
                s += ((num / den) * 100) + ",";

                num = data['PDC-058'].anonymous[k]["aggregate_result"]["numerator"];
                den = data['PDC-058'].anonymous[k]["aggregate_result"]["denominator"];
                ...
                logger.warn("Unable to get data for anonymous provider, will not list them in this report.");
            }
