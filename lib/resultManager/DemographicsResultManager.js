 ...
 *   object.
 */
function DemographicsResultManager(id, data, init, proc) {
    ...
    var generateResult = function () {
        ...
        var group     = that.getGroup('Attachment');
        ...
        //apply patient privacy filter (zeros any values that are less than 5)
        clinician = proc.applyPrivacyFilter(clinician);
	
        group     = proc.applyPrivacyFilter(group);
	
        network   = proc.applyPrivacyFilter(network);

	//console.log('CLINICAN: \n' + util.inspect(clinician, false, null));
	//console.log('GROUP: \n' + util.inspect(group, false, null));
	//console.log('NETWORK: \n' + util.inspect(network, false, null));
	
	var fault = false;
	
        //check that all results back were valid.
        if (!clinician) {
	  logger.error(new Error('clinician evaluates to false').stack);
	  fault = true;
	}
	if(!group) {
	  logger.error(new Error('group evaluates to false').stack);
	  fault = true;
	}
	if(!network)
	{
	  logger.error(new Error('network evaluates to false').stack);
	  fault = true;
	}
	if(!proc.data.time) {
	  logger.error(new Error('data.time evaluates to false').stack);
          fault = true;
        }
    ...
    /**
     * Applies a privacy filter to the demographics data by calling proc.privacyFilter().
     *
     * @param input {Object} - demographics data object. Has structure like:
     *      { GENDER : { AGE_STRING : Number, ... }, ... }
     *
     * @return {object} - input data object with all values filtered. Returns null if the input was invalid or not formed as expected.
     */
    var applyPrivacyFilter  = function (input) {
        ...
        for (var g = 0; g < proc.supportedGenders.length; g++) {

            gender = proc.supportedGenders[g];

            for (var a = 0; a < proc.supportedAgeRanges.length; a++) {

                age = proc.supportedAgeRanges[a];

                try {

                    input[gender][age] = proc.privacyFilter(input[gender][age]);

                } catch (e) {

                    //if we catch an error, it means that we had an invalid input format.
                    logger.error(new Error('applyPrivacyFilter() caught an error').stack);
                    return null;
                }
    ...
    proc.supportedGenders          = SUPPORTED_GENDERS;
    proc.applyPrivacyFilter = applyPrivacyFilter;
