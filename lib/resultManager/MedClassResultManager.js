 ...
 *   object.
 */
function MedClassResultManager(id, data, init, proc) {
    ...
    var generateResult = function (input, next) {
        ...
        var finalResult = {
            ...
            'processed_result': {
                display_names: {
                    clinician: "clinician",
                    group  : "group (" + groups.findGroup(id) + ")",
                    network: "network"
                }
            }
        };
        ...
        networkResults = proc.applyPrivacyFilter(networkResults);
        groupResults   = proc.applyPrivacyFilter(groupResults);
        selfResults    = proc.applyPrivacyFilter(selfResults);
    ...
    var getFormattedData = function (next) {
        ...
        getDrugClasses(that.getNetwork(), function (err) {
            ...
            if (err) {
                ...
                return generateResult(that.getNetwork(), next);
    ...
    var lookUpDrugCode = function (drug, next) {
        ...
        if (drug.codeSystem.toLowerCase() === "hc-din") {
            ...
            //error case.
            logger.warn("Could not find mapping for code system: " + drug.codeSystem + " classifying as UNCLASSIFIED");
        ...
        request.get(
            ...
            function (error, res, body) {
                ...
                if (error) {
                    ...
                    var m = "Request to: " + process.env.DCLAPI_URI + "/" + path + "/" + drug.code + " failed with response code: " + res.statusCode + " using UNCLASSIFIED for this type";
                    logger.warn(m);
    ...
    /**
     * Applies the privacyFilter() function.
     *
     *
     * @param input {Array} - the medication object to filter, hsa structure like:
     *  [ { "class" : "XX", "count" : X }, ... ]
     *
     * @return {Array} - same object as the input with the "count" fields filtered.
     *  if there is an error or the input is invalid, null is returned
     */
    var applyPrivacyFilter = function (input) {
        ...
        try {
            ...
            for (var i = 0; i < input.length; i++) {
                ...
                if (!input[i] || input[i].count === undefined) {
                ...
                input[i].count = proc.privacyFilter(input[i].count);

            }

        } catch (e) {
            logger.error("applyPrivacyFilter(Array) caught an error: " + e);
