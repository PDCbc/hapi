...
var groups = [
    {
      name:'PPhRR1',
      members:['cpsid','27542', 'oscar', 'osler' ]
    }
  ];
...
function groupFilter(req, group, fields)
{
  ...
  console.log('pass: ');
  console.log((group.members.indexOf(fields[1]) !== -1));
...
function process(req, res, group, query_aggregate_result, level)
{
    ...
    function(key)
    {
      ...
      if(fields[0] !== 'time')
      {
        ...
        if(filters[level](req, group, fields))
        {
          ...
          console.log('result there: ' + result.aggregate_result[fields[0]]);
  ...
  console.log(level + ' result:' + util.inspect(result, false, null));
  ...
  data.models.query
    ...
    .select("_id title description executions").exec(
      function (err, queries) {
        ...
        logger.success("fetch_data("+req.params.title+") for clinician: "+ req.session.user.clinician);
        ...
        console.log('RESULT: ' + util.inspect(res.result, false, null));
        res.json(
          { processed_result:
              res.result,
              network_id:"PDC",
