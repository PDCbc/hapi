          ...
          data.models.query.find({title:req.params.title})
            ...
            for(var i=0; i<queries.length; i++)
            {
              for(var j=0; j<queries[i].executions.length; j++)
              {
                ...
                typeof queries[i].executions[j].aggregate_result.denominator_patients_above_19 !== 'undefined')
                {
                  ...
                  aggregate_result.numerator_has_recorded_values = queries[i].executions[j].aggregate_result.numerator_has_recorded_values;
                  aggregate_result.denominator_patients_above_19 = queries[i].executions[j].aggregate_result.denominator_patients_above_19;
            ...
            data.models.result.find({execution_id: {$in : execution_ids}, endpoint_id: req.query.endpoint_id})
              ...
              function (err, results) {
                ...
                var numerator_key = "numerator_has_recorded_values_" + req.query.provider_id;
                var denominator_key = "denominator_patients_above_19_" + req.query.provider_id;
                ...
                for(var i=0; i<results.length; i++)
                {
                  if(results[i].value._id)//exclude empty values
                  {
                    ...
                    data.models.processed_result.clinician.push(
                      {time: new Date(results[i].value.created_at).toDateString(),
                        aggregate_result:
                          {numerator_has_recorded_values: results[i].toJSON().value[numerator_key],
                            denominator_patients_above_19: results[i].toJSON().value[denominator_key]}
                      }
                    );
                  }
                }
