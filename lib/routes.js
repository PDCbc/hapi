  ...
  data.models.query
    ...
    .select("_id title description executions").exec(
      function (err, queries) {
        ...
        logger.info(util.inspect(exe, false, null));

        var rrm = RatioResultManager( req.session.user.clinician, exe )

        rrm.getFormattedData( function( result ){ 

          logger.success(util.inspect(result, false, null), "FINAL RESULT");
