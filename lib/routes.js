                    ...
                    if (!err && isMatch)
                    {
                      user.roles(
                        function respond(err, roles)
                        {
                          ...
                          //hack for now
                          user.endpoint='54f61f04f2ba32ae00000002';
                          user.provider='cpsid';
      ...
      function (err, queries) {
        ...
        query.executions.forEach(
          function(x)
          {
            ...
            Object.keys(x.aggregate_result).forEach(
              function (x)
              {
                console.log('key: ' + key);
                ...
                {
                  aggregate_result.denominator += x.aggregate_result[key];
                }
                else if(fields[0] === 'numerator')
                {
                  aggregate_result.numerator += x.aggregate_result[key];
                }
              }
            );


            console.log('den: ' + aggregate_result.denominator);
            console.log('num: ' + aggregate_result.numerator);
