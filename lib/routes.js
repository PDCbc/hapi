  ...
  data.models.query
    ...
    .exec(
      function (err, queries) {
        ...
        console.log('fetch_executions_data: ' + req.params.title);

        data.models.processed_result.network = [];

        if(queries.length != 1)
        {
          logger.error('there is not exactly 1 query for **' + req.params.title + '**');
          return;
        }

        if(queries[0].executions === null || queries[0].executions===undefined)
        {
          logger.error('query executions null or undefined for **' + req.params.title + '**');
          return;
        }

        if(queries[0].executions.length<1)
        {
          logger.error('no query executions for **' + req.params.title + '**');
          return;
        }

        queries[0].executions.forEach(
          function (x){
            console.log('execution: ' + util.inspect(x, false, null));
        ...
        console.log('execution_ids: ' + util.inspect(execution_ids));
...
function fetch_provider_data(req, res, data, execution_ids)
{
  ...
  data.models.result.find({execution_id: {$in : execution_ids}, endpoint_id: req.params.endpoint})
    ...
    function (err, results) {
      ...
      {
        logger.error('results null or undefined');
        return;
      }
      ...
      console.log('execution_ids in fpd: ' + util.inspect(execution_ids, false, null));
      ...
      for(var i=0; i<results.length; i++)
      {
        console.log('result: ' + util.inspect(results[i], false, null));
