    ...
    router.route('/:title').get(
        ...
        function (req, res) {
            ...
            try {
			    ...
			    //shouldn't assume these are sorted
			    ...
			    exes = exes.sort( function(a, b)
						...
						return 0;
                            ...
                            if(!queries[0].panels || queries[0].panels.length < 1){
                                logger.warn("Unclear which panel this query belongs to!");
                                res.status(204);
                                return res.json({});
                            }

                            var initiative = queries[0].panels[0];
                            ...
                            switch (queryTypes.getQueryType(req.params.title)) {
                                ...
                                case queryTypes.RATIO:
                                    ...
                                    rrm = RetroRatioResultManager(req.session.user.clinician, JSON.parse(JSON.stringify(exes)), initiative);
                                    break;

                                case queryTypes.DEMOGRAPHICS:
                                    rrm = RetroDemographicsResultManager(req.session.user.clinician, JSON.parse(JSON.stringify(exes)), initiative);
                                    break;
