...
var AttachedActivePatientReport = function (data, dayOfMonth, proc) {
    ...
    var output = function(previousValue, currentValue, currentIndex, array, headerFlag) {
      ...
      if( rowId !== lastPid ){
        ...
        // Starting a new row, so make sure there's no previous patient count
      ...
      previousValue += ',' + currentValue.date.toDateString() + ',' +
        currentValue.numerator + ',' +
        ((lastNumerator !== undefined) ? currentValue.numerator - lastNumerator : '')  + ',' +
