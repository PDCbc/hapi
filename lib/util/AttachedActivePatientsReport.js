...
var AttachedActivePatientReport = function (data, dayOfMonth, proc) {
    ...
    var output = function(previousValue, currentValue, currentIndex, array, headerFlag) {
      ...
      // If writing first column, then cleanup hashes and print
      if( rowId !== lastPid ){
      ...
      else
      {
        ...
        if( headerFlag && firstPhysician ) {
          header += lastDate + ',' + 'Active Patients,' + 'Change in Active Patients,' + '% Change in Active Patients,';
        }

        // Calculate, based heavily on last values
        previousValue += ',' + lastDate + ',' +
          lastNumerator + ',' +
          ((lastNumerator !== undefined) ? lastNumerator - currentValue.numerator : '')  + ',' +
          ((lastNumerator !== undefined && lastDenominator !== undefined) ? ((lastNumerator/lastDenominator) - (currentValue.numerator/currentValue.denominator)) : '');
      }

      // Iterate values
      lastDate = currentValue.date.toDateString();
