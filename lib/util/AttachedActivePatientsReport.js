...
<<<<<<< HEAD
            tmpDate = new Date(Math.floor(that.raw_data[i]['time']*1000));

            if (tmpDate.getDate() === that.dayOfMonth) {

                console.log("Including execution from date: " + tmpDate + " status : " + that.raw_data[i].status);
                l = Execution(that.raw_data[i].aggregate_result, tmpDate, null);
                that.exes.push(Execution(that.raw_data[i].aggregate_result, tmpDate, null));
