...
var getMembers = require('../groups.js').getMembers;
...
var MasterReport = function (executions, dayOfMonth, proc) {
  ...
  proc.dayOfMonth = dayOfMonth || undefined; 

  if(proc.dayOfMonth === undefined) {
    if(process.env.EXECUTION_DATE===undefined) {
        throw new Error('EXCUTION_DATE not specified');
    }

    proc.dayOfMonth = Number.parseInt(process.env.EXECUTION_DATE);
  }
  ...
  if(getMembers(process.env.GROUP).length<1) {
    throw new Error('GROUP ' + process.env.GROUP + ' is not configured or the group has no members');
  }
  ...
  var filterResultsForExecutionDate = function (e) {//right day filter
    ...
    if(keys.length!=1) {
      throw new Error('found execution with not one and only one key');
    }

    var parsedKey = JSON.parse(keys[0]);
    var date = new Date(parsedKey.time*1000);

    return date.getDate() === proc.dayOfMonth;
  };
  ...
  */
  ...
  var getReport = function (withProviders) {
	...
	inTheGroup.forEach(function(x){console.log(x);});

        console.log('####notInTheGroup####');
	notInTheGroup.forEach(function(x){console.log(x);});
  ...
  function filterIndividualClusteredResultsForGroup(physician) {
    ...
    physInGroup ? inTheGroup.push(pid) : notInTheGroup.push(pid);
