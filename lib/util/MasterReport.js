...
var MasterReport = function (executions, dayOfMonth, proc) {
    ...
    for (var i = 0; i < executions.length; i++) {
    	...
    	Execution(executions[i].aggregate_result, executions[i].time, proc);//structure and drop the executions into the proc
...
*/
	...
	proc.results = proc.results.filter(filterResults);//filter wrong day executions
	...
	proc.results[NETWORK] = clone(proc.results[INDIVIDUAL]);//deep clone just to be safe
	
	//handle individuals
	proc.results[INDIVIDUAL] = proc.results[INDIVIDUAL].filter(
	    function groupFilter(physician) {
	...
	);//filter out physicians which are not part of this group
	...
	if(keys.length%40!==0){
      console.log(util.inspect(aggregate, false, null));
	...
	var individualCSVSection = individual_results.reduce(
	  function (previousAtPhysician, physician, physicianIndex, physicianArray) {
	    console.log('####SOURCE####');
	    console.log('physicianIndex0: ' + physicianIndex);
	    
	    return physician.reduce(
	      function (previousAtTimeInterval, timeInterval, timeIntervalIndex, timeIntervalArray) {
		console.log('physicianIndex1: ' + physicianIndex);
	...
	var groupCSVSection = group_results.reduce(reduceTimeIntervalsGroup, []);
	
	var networkCSVSection = network_results.reduce(reduceTimeIntervalsNetwork, []);
	...
	console.log('physicianIndex2: ' + physicianIndex);
	...
	console.log('physicianIndex3: ' + physicianIndex);
	...
	console.log(util.inspect(mapCurrentValue, false, null));
	
	mapCurrentValue = mapCurrentValue.reduce(function(previousValue, currentValue, currentIndex, array) {
							console.log('currentValue: ' + util.inspect(currentValue, false, null));
	...
	//mapCurrentValue[0] = removeTimeKeys(mapCurrentValue[0]);
	...
	return previousValue;
	 ...
	 return clusterByField(previousValue, currentValue, currentIndex, array, 'time');
      ...
      function removeTimeKeys(entity) {		
	return Object.keys(entity).map(
		function (key) {
			timeKey = key;
			return entity[key];
		});
      }//remove time keys transorming the object back to an array
	...
	{
	  console.log('cbfcv:');
	  console.log(clusterByFieldCurrentValue);
	  console.log('STACK\n' + new Error().stack);
