...
var reportTitles = {'PDC-001':'Demographics', 'PDC-1740':'Encounters', 'PDC-1738':'AttachedPatients'};
...
process.env.HAPI_GROUPS = process.env.HAPI_GROUPS || ('/home/app/groups/groups.json');
process.env.GROUP = process.env.GROUP || undefined;
...
var doReport = function (data, connection) {
    ...
    if (process.env.QUERY === 'PDC-001' || process.env.QUERY === 'PDC-1740') {
        ...
        throw new Error('unknown query title : ' + process.env.QUERY + ' failed to generate report.');
    }
    ...
    var month = date.getMonth() + 1;//take care of js nonesense
    var date = date.getDate();

    fs.writeFileSync(process.env.GROUP + reportTitle + 'Report' +  year + '-' + month + '-' + date + '.csv', s);
    var fileName = process.env.GROUP + reportTitles[process.env.QUERY] + 'Report.csv';
